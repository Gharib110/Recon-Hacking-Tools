	...
	go func() {
		...
		for i := start; i <= stop; i++ {
			...
			default:
				...
				if block, err = New(key[3:], 40); err != nil {
					log.Fatalln(err)
				}
				data = &CryptoData{
					block: block,
...
func main() {
	...
	*/
	...
	var minimum, maximum, prods = uint64(0x0000000000), uint64(0xffffffffff), uint64(75)
	var step = (maximum - minimum) / prods

	done := make(chan struct{})
	work := make(chan *CryptoData, 100)
	if (step * prods) < maximum {
		step += prods
	}
	var start, end = minimum, minimum + step
