...
var (
	snapshotLen = int32(320)
	promiscuous = true
	timeout     = pcap.BlockForever
	//The following line indicates just captures packets with ACK/FIN, ACK, ACK/PSH
	filter   = "tcp[13] == 0x11 or tcp[13] == 0x10 or tcp[13] == 0x18"
	devFound = false
	results  = make(map[string]int)
)

func buildPortList(portString string) ([]string, error) {
	ret := make([]string, 0)
...
func ScanBeyondSYNFloodProtection() {
	if len(os.Args) != 4 {
	...
	ports, err := buildPortList(os.Args[3])
	if err != nil {
		log.Panicln(err)
	}
